...
CREATE TABLE `User` (
    ...
    `info` JSON NULL,
    `videoTotalStorage` DECIMAL(65, 30) NOT NULL DEFAULT 80,
    `videoUsedStorage` DECIMAL(65, 30) NOT NULL DEFAULT 0,
...
CREATE TABLE `Request` (
    ...
    UNIQUE INDEX `Request_fromUserId_toUserId_toGroupId_key`(`fromUserId`, `toUserId`, `toGroupId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Video` (
    ...
    `videoSize` DECIMAL(65, 30) NOT NULL,
    `numberOfViews` INTEGER NULL,
    `viewsCost` DECIMAL(65, 30) NULL,
    `ownerUserId` INTEGER NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `VideoViews` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `numberOfViews` INTEGER NOT NULL DEFAULT 1,
...
-- AddForeignKey
ALTER TABLE `Request` ADD CONSTRAINT `Request_toGroupId_fkey` FOREIGN KEY (`toGroupId`) REFERENCES `Group`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Video` ADD CONSTRAINT `Video_ownerUserId_fkey` FOREIGN KEY (`ownerUserId`) REFERENCES `User`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `_GroupToVideo` ADD FOREIGN KEY (`A`) REFERENCES `Group`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_GroupToVideo` ADD FOREIGN KEY (`B`) REFERENCES `Video`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
